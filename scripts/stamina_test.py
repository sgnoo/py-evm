...
def deposit(chain):
    ...
    # 1. make message
    # 2. apply message
    # NOTE: message_gas는 어떻게 처리했는지. 그냥 max 값을 줬는지?
    vm_state = chain.get_vm().state
    stamina = b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xde\xad'

    encoded_function_signature = keccak("deposit(address)".encode())
    function_signature = encoded_function_signature[0:4]
    encoded_parameter = encode_single('address', encode_hex(FUNDED_ADDRESS))
    data = function_signature + encoded_parameter

    contract_address = None
    transaction_context = BaseTransactionContext(0, FUNDED_ADDRESS)
...
def get_delegatee(chain):
    ...
    vm_state = chain.get_vm().state
    stamina = b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xde\xad'
    data = b'\x9bNs_\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xf9\xab\xaf\x1d\xbb\xe5@\xa2\xc8\xc7\xa4\xc1\xe5\xb3\t;\x94X\xd6\xc0'
    contract_address = None
    transaction_context = BaseTransactionContext(0, b'\xa9OSt\xfc\xe5\xed\xbc\x8e*\x86\x97\xc1S1g~n\xbf\x0b')

    # TODO: gas, sender
    message = Message(
        gas=200000,
        to=stamina,
        sender=b'\xa9OSt\xfc\xe5\xed\xbc\x8e*\x86\x97\xc1S1g~n\xbf\x0b',
        value=0,
        data=data,
        code=code,
        create_address=contract_address,
...
def get_delegatee_stamina(account_db, transaction, delegatee):
    # 1. make message
    # 2. apply message
    # NOTE: message_gas는 어떻게 처리했는지. 그냥 max 값을 줬는지?
    vm_state = chain.get_vm().state
...
def substract_stamina_function(chain):
    # 1. make message
    # 2. apply message
    # NOTE: message_gas는 어떻게 처리했는지. 그냥 max 값을 줬는지?
    vm_state = chain.get_vm().state
